...
public class PacketRemoteChannelsRouterReady extends PacketListFromServer<PacketRemoteChannelsRouterReady, ControllerChannelClientInfo> {
    ...
    public static class Handler implements IMessageHandler<PacketRemoteChannelsRouterReady, IMessage> {
        @Override
        public IMessage onMessage(PacketRemoteChannelsRouterReady message, MessageContext ctx) {
            XNet.proxy.addScheduledTaskClient(() -> handle(message, ctx));
            return null;
        }

        private void handle(PacketRemoteChannelsRouterReady message, MessageContext ctx) {
