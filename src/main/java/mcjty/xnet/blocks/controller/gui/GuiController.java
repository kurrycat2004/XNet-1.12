...
public class GuiController extends GenericGuiContainer<TileEntityController> {
    ...
    protected void drawGuiContainerBackgroundLayer(float v, int x1, int x2) {
        ...
        temporaryDrawStackTooltips(x1, x2);
    }

    private void temporaryDrawStackTooltips(int mouseX, int mouseY) {
        ...
        if (widget instanceof BlockRender) {
            ...
            if (!itemStack.isEmpty()) {
                temporaryCustomRenderToolTip(blockRender, itemStack, mouseX, mouseY);
            }
        }
    }

    private void temporaryCustomRenderToolTip(BlockRender blockRender, ItemStack stack, int x, int y) {
        List<String> list;
